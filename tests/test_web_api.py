...
# The web UI is optional and rides on SQLAlchemy. When it isn't installed,
# skip this whole module cleanly rather than erroring out collection.
pytest.importorskip("sqlalchemy", reason="web UI optional dep not installed")
...
def server_url() -> str:
    ...
    os.environ["PROMETHEAN_WEB_DB"] = db_path
    os.environ["PROMETHEAN_WEB_SECRET"] = "test-secret-do-not-use-in-prod"
    os.environ["PROMETHEAN_LOG_LEVEL"] = "WARNING"  # quiet during tests
...
def test_metrics_prometheus_format(server_url):
    with _client(server_url) as c:
        ...
        assert r.status_code == 200
        body = r.text
        assert "promethean_uptime_seconds " in body
        assert "promethean_requests_total " in body
        assert "# TYPE promethean_requests_total counter" in body
...
def test_metrics_counters_increment(server_url):
    with _client(server_url) as c:
        ...
        assert _counter(after, "promethean_requests_total") > _counter(
            before, "promethean_requests_total")
        assert _counter(after, "promethean_requests_4xx") >= _counter(
            before, "promethean_requests_4xx") + 1
        assert _counter(after, "promethean_auth_registrations_total") >= 1
