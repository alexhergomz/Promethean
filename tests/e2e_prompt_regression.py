...
def _generate_masked_prompt(tmp_path, monkeypatch) -> str:
    ...
    # The agent_tools/codeact block is optional (gated on installed graph
    # deps) AND embeds an absolute repo path, so it varies by machine and CI.
    # Force it off like every other optional block to keep the golden stable.
    monkeypatch.setattr(_context, "_agent_tools_available", lambda: False)
...
def _regenerate() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        with mock.patch.object(_context, "get_memory_context", return_value=""), \
             ...
             mock.patch.object(_context, "_agent_tools_available", return_value=False):
            import os
            prev_cwd = os.getcwd()
            try:
                os.chdir(tmp)
