...
def test_write_new_file_shows_preview(tmp_path):
    """New-file Write returns an all-additions unified diff so the renderer
    can show a content preview. Previously the result was just a one-line
    'Created' summary with no diff."""
    from tools import _write
    f = tmp_path / "new.txt"
    result = _write(str(f), "first line\nsecond line\n")
    assert "Created" in result
    assert "---" in result            # synthetic diff header present
