...
def test_max_tool_output_config_is_live():
    """The max_tool_output config governs the ceiling for every tool (it used
    to be ignored — a dead config). config wins over the param default."""
    def big_func(params: dict, config: dict) -> str:
        return "y" * 5000
    register_tool(ToolDef(
        name="big2",
        schema={"name": "big2", "description": "b", "input_schema": {"type": "object", "properties": {}}},
        func=big_func, read_only=True, concurrent_safe=True,
    ))
    # No max_output arg: the config value must be applied.
    result = execute_tool("big2", {}, config={"max_tool_output": 200})
    assert len(result) < 5000
    assert "truncated" in result
    # And a generous config leaves it untouched.
    full = execute_tool("big2", {}, config={"max_tool_output": 100000})
    assert full == "y" * 5000
