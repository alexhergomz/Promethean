...
class TestReplVoiceIntegration:
    def test_voice_in_commands(self):
        import promethean
        assert "voice" in promethean.COMMANDS

    def test_voice_command_callable(self):
        import promethean
        assert callable(promethean.COMMANDS["voice"])
    ...
    def test_handle_slash_voice_sentinel(self):
        ...
        original_voice = promethean.COMMANDS.get("voice")
        try:
            with patch.object(promethean, "cmd_voice", return_value=sentinel):
                # Re-bind in COMMANDS so the patch is seen
                promethean.COMMANDS["voice"] = promethean.cmd_voice
                result = promethean.handle_slash("/voice", object(), {})
                assert result == sentinel
        finally:
            if original_voice is not None:
                promethean.COMMANDS["voice"] = original_voice
    ...
    def test_voice_status_no_crash(self, capsys):
        ...
        # Should not raise
        try:
            promethean.cmd_voice("status", object(), {})
        except SystemExit:
            pass
    ...
    def test_voice_lang_set(self, capsys):
        import promethean
        promethean.cmd_voice("lang zh", object(), {})
        assert promethean._voice_language == "zh"
        # Reset
        promethean.cmd_voice("lang auto", object(), {})
        assert promethean._voice_language == "auto"
