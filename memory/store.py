...
Storage layout:
  user scope    : ~/.promethean/memory/<slug>.md
  project scope : .promethean/memory/<slug>.md  (relative to cwd)
...
USER_MEMORY_DIR = Path.home() / ".promethean" / "memory"
...
def get_project_memory_dir() -> Path:
    ...
    return Path.cwd() / ".promethean" / "memory"
...
def get_memory_dir(scope: str = "user") -> Path:
    ...
    Args:
        scope: "user" (global ~/.promethean/memory) or
               "project" (.promethean/memory relative to cwd)
    """
    if scope == "project":
