"""
commands/session.py — Session management commands for Promethean.
...
def cmd_cloudsave(args: str, state, config) -> bool:
    ...
    if sub == "list":
        info("Fetching your promethean sessions from GitHub Gist…")
        ...
        for s in sessions:
            ...
            desc = s["description"].replace("[promethean]", "").strip()
...
def _shutdown_subagents(timeout_s: float = 15.0) -> None:
    """Cancel any alive rabbit-hole / background subagents and join the
    SubAgentManager pool. Ensures the process exits cleanly without
    leaving worker threads hanging — and triggers synthesis on each
    rabbit-hole as a side effect (cancel → finally block → final.md).

    Best-effort: any failure is silenced; we want exit to proceed even
    if cleanup hits an unexpected error.
    """
    try:
        from multi_agent.tools import get_agent_manager
        mgr = get_agent_manager()
        alive = [t for t in mgr.list_tasks()
                 if t.status in ("running", "idle")]
        if alive:
            rabbit = [t for t in alive
                      if getattr(t, "rabbit_hole_dir", "")]
            if rabbit:
                info(f"  Cancelling {len(rabbit)} running rabbit-hole(s) "
                     f"(synthesis writes final.md)...")
            for t in alive:
                mgr.cancel(t.id)
            # Give the finally blocks ~timeout_s to write reports.
            import time as _t
            deadline = _t.time() + timeout_s
            while any(t.status in ("running", "idle") for t in alive) \
