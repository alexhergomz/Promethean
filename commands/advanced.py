"""
commands/advanced.py — Advanced power commands for Promethean.
...
def _ssj_trading_submenu(config, state):
    ...
    if choice in ("0", "q", ""):
        ...
        return ("__ssj_query__",
                f"You are the Promethean Trading Agent. Analyze each symbol in this watchlist: {watchlist}. "
                f"For each symbol:\n"
...
def cmd_mcp(args: str, _state, config) -> bool:
    ...
    if not servers:
        ...
        if not configs:
            ...
            info("Add servers in ~/.promethean/mcp.json or .mcp.json")
