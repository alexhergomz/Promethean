...
def _get_skill_paths() -> list[Path]:
    return [
        Path.cwd() / ".promethean" / "skills",   # project-level (priority)
        Path.home() / ".promethean" / "skills",   # user-level
    ]
...
def skills_catalog(max_skills: int = 40) -> str:
    """Render a compact catalog of available skills for the system prompt.

    Lists each skill's name, argument hint, and when-to-use/description so the
    model can decide to invoke a skill (via the Skill tool) without first
    calling SkillList. Mirrors how Claude Code / Hermes surface skills as a
    discoverable, model-invokable capability set. Returns "" if no skills.
    """
    try:
        skills = load_skills()
    except Exception:
        return ""
    # Only skills the model should consider invoking.
    skills = [s for s in skills if (s.when_to_use or s.description)]
    if not skills:
        return ""

    lines: list[str] = []
    for s in sorted(skills, key=lambda x: x.name)[:max_skills]:
        hint = (s.when_to_use or s.description or "").strip().replace("\n", " ")
        if len(hint) > 160:
            hint = hint[:157] + "..."
        arg = f" {s.argument_hint}" if s.argument_hint else ""
        lines.append(f"- **{s.name}**{arg} — {hint}")
    return "\n".join(lines)
