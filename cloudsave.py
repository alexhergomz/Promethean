"""
Cloud sync for promethean sessions via GitHub Gist.
...
Config keys (stored in ~/.promethean/config.json):
...
def upload_session(
    ...
    filename = f"promethean_session_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def list_sessions(token: str, max_results: int = 20) -> tuple[list[dict], str | None]:
    """
    List Gists tagged as promethean sessions.
