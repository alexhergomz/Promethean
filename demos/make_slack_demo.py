#!/usr/bin/env python3
"""
Generate animated GIF demo of promethean Slack Bridge.
...
def draw_slack(img, messages):
    ...
    d.text((main_x + 24, py + 8), "promethean", font=FONT_B, fill=(30, 30, 30))
    d.text((main_x + 12, py + 26), "promethean bot channel", font=FONT_XS, fill=(100, 100, 100))
    ...
    d.text((main_x + 20, input_y + 12), "Message #promethean", font=FONT_SM, fill=(180, 180, 180))
...
def prompt_line(text="", cursor=False):
    cur = "█" if cursor else ""
    return [seg("[promethean] ", SUBTEXT), seg("» ", CYAN, True), seg(text + cur, TEXT)]
...
def build_scenes():
    ...
    slack_init = [
        (BOT_NAME, "🟢 promethean is online. Send me a message and I'll process it.", True, False),
    ]
    ...
    resp1_lines = [
        ...
        "",
        "  promethean.py   — Main REPL + slash commands",
    ...
    slack_r1 = slack_q1 + [
        (BOT_NAME, "Here are the files: promethean.py, agent.py, tools.py, providers.py, config.py …", True, False),
    ]
    ...
    add(q3_base + [
        ...
        tool_ok("Found in promethean.py:480"),
        None,
        tool_line("⚙", "Read", "promethean.py:480-550", CYAN),
    ...
    slack_stopped = slack_stop + [(BOT_NAME, "🔴 promethean bridge stopped.", True, False)]
