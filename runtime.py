"""
runtime.py — Live session context for Promethean.
...
class RuntimeContext:
    ...
    # File-context tracker — Cline-style. For each absolute file path the
    # model has Read this session, store (mtime, size, turn_seen) so the
    # next Read of an unchanged file can be elided to a stub. Stops the
    # "model re-reads files it already saw" loop on long sessions.
    file_tracker: dict = field(default_factory=dict)
