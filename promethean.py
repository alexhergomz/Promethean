#!/usr/bin/env python3
"""
Promethean — A Fast, Easy-to-Use, Python-Native Personal AI Assistant for Any Model, Built to Work for You Autonomously 24/7.

Usage:
  python promethean.py [options] [prompt]
...
Slash commands in REPL:
  ...
  /cloudsave list   List your promethean Gists
...
from commands.core import (
    ...
    cmd_graph_view, cmd_optillm, cmd_undo,
)
...
from commands.slots_cmd import cmd_slots
...
def _read_version() -> str:
    ...
    try:
        ...
        return _pkg_version("promethean")
    except Exception:
        return "0.0.0"
...
COMMANDS = {
    ...
    "reports":     cmd_reports,
    "rabbit-hole": cmd_rabbit_hole,
    "rabbithole":  cmd_rabbit_hole,
    "rh":          cmd_rabbit_hole,
    "slots":       cmd_slots,
    "compact":     cmd_compact,
    "graph-view":  cmd_graph_view,
    "graphview":   cmd_graph_view,
    "init":        cmd_init,
    "export":      cmd_export,
    "copy":        cmd_copy,
    "status":      cmd_status,
...
def __getattr__(name: str):
    ...
    external code and tests can access promethean.cmd_voice and
    promethean._voice_language without the voice module being hard-coded here.
    """
    if name == "cmd_voice":
    ...
    if name == "_voice_language":
        try:
            ...
            return "auto"
    raise AttributeError(f"module 'promethean' has no attribute {name!r}")
...
_CMD_META: dict[str, tuple[str, list[str]]] = {
    ...
    "model":       ("Show / set model",                   ["qwen", "m2", "m2-fast", "m2-opti"]),
    "api":         ("Set/show provider API key",          ["minimax", "anthropic", "openai", "deepseek"]),
    "failover":    ("Manage failover ladder",             ["off", "add", "qwen", "m2"]),
    ...
    "undo":        ("Revert last N file mutations",       ["1", "2", "5", "list", "confirm"]),
    "optillm":     ("Pick OptiLLM technique",              ["off", "moa", "mcts", "bon", "mars", "cepo"]),
    ...
    **({"video": ("AI video factory: story→voice→images→mp4", ["status", "niches"])} if _VIDEO_AVAILABLE else {}),
    ...
    "rabbit-hole": ("Long-running autonomous research agent",
...
def setup_readline(history_file: Path):
    ...
    # Belt-and-suspenders subagent cleanup. The /quit and Ctrl-C paths
    # already call _shutdown_subagents, but this catches any other exit
    # path (uncaught exception, sys.exit from a corner case, etc.) so
    # rabbit-hole threads never outlive the REPL process.
    def _atexit_shutdown_subagents():
        try:
            from commands.session import _shutdown_subagents
            _shutdown_subagents(timeout_s=5.0)
        except Exception:
            pass
    atexit.register(_atexit_shutdown_subagents)
...
def repl(config: dict, initial_prompt: str = None):
    ...
    def _status_footer(state, config) -> str:
        ...
        Returns ANSI text. Cheap — runs on every prompt_toolkit redraw, so
    ...
    if not initial_prompt:
        ...
        # ── Promethean startup: a flame in solid square blocks, one colour ──
        _FLAME = [
            "   █   ",
            "  ███  ",
            " █████ ",
        ...
        try:
            for i in range(8):
                ...
                sys.stdout.write(f"\r  {clr(frame, 'flame', 'bold')} Igniting Promethean...")
                sys.stdout.flush()
                time.sleep(0.12)
        ...
        # Print the flame mark — solid flame colour, no gradient.
        for _ln in _FLAME:
            print("  " + clr(_ln, "flame", "bold"))
        print()

        model    = config["model"]
        pname    = detect_provider(model)
        model_clr = clr(model, "ember", "bold")
        prov_clr  = clr(f"({pname})", "dust")
        pmode     = clr(config.get("permission_mode", "auto"), "warmth")
        ver_clr   = clr(f"v{VERSION}", "ember")
    ...
    def run_query(user_input: str, is_background: bool = False):
        ...
        with query_lock:
            ...
            print(clr("\n╭─ Promethean ", "dust") + clr("▲", "flame") + clr(" ─────────────────────────", "dust"))
    ...
    while True:
        ...
        try:
            ...
            print()
            # Cancel any alive subagents so synthesis runs on rabbit-holes
            # and worker threads don't outlive the REPL. Without this,
            # Ctrl-C left rabbit-hole threads spinning past process exit
            # (or the interpreter hung waiting on non-daemon threads).
            try:
                from commands.session import _shutdown_subagents
                _shutdown_subagents()
            except Exception:
...
def main():
    # Rebrand migration: relocate a pre-rebrand ~/.cheetahclaws dir to
    # ~/.promethean before anything reads config. Best-effort, idempotent.
    try:
        import cc_config as _ccfg
        _ccfg.migrate_legacy_config_dir()
    except Exception:
        pass

    parser = argparse.ArgumentParser(
        prog="promethean",
        description="Promethean — minimal Python Claude Code implementation",
        add_help=False,
    )
