#!/usr/bin/env bash
# ── Promethean Installer ────────────────────────────────────────────────
# curl -fsSL https://raw.githubusercontent.com/alexhergomz/Promethean/main/scripts/install.sh | bash
#
...
REPO="https://github.com/alexhergomz/Promethean.git"
INSTALL_DIR="$HOME/.promethean-src"
MIN_PYTHON="3.10"
...
# ── Banner ───────────────────────────────────────────────────────────────
B1=$(center "Promethean Installer" $BOX_W)
...
# ── Install with pip ───────────────────────────────────────────────────
info "Installing Promethean..."

VENV_DIR="$HOME/.promethean-venv"
USE_VENV=false
...
if command -v promethean &>/dev/null; then
    ok "promethean is on PATH"
elif [ -f "$BIN_DIR/promethean" ]; then
    SHELL_RC=""
    ...
    warn "promethean not found on PATH — you may need to add pip's bin directory manually."
fi

# ── Print version ──────────────────────────────────────────────────────
VERSION=$(promethean --version 2>/dev/null || echo "installed")
...
echo ""
echo -e "    ${RELOAD_CMD}"
echo -e "    promethean        ${DIM}# start the REPL${RESET}"
echo ""
echo -e "  ${DIM}First run will guide you through setup (API key, model).${RESET}"
echo -e "  ${DIM}Or run: promethean --setup${RESET}"
echo ""
