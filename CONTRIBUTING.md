...
> The Python package, CLI, and config dir are all named `promethean` (`promethean.py`, `~/.promethean/`). It's a fork of the [cheetahclaws](https://github.com/SafeRL-Lab/cheetahclaws) harness — see the README credits.
...
```
promethean/
├── promethean.py          # REPL loop, slash commands, readline setup
├── agent.py                 # LLM turn loop, retries, permission checks
├── providers.py             # API streaming (Anthropic, OpenAI, Ollama, etc.)
├── config.py                # User config (load/save to ~/.promethean/config.json)
...
**`config` dict** — serializable user settings loaded from `~/.promethean/config.json`. Contains model name, API keys, permission mode, etc. Saved to disk by `save_config()`.
...
Plugins live in `~/.promethean/plugins/<name>/` (user scope) or `.promethean/plugins/<name>/` (project scope). Installed via `/plugin install name@<url>`.
