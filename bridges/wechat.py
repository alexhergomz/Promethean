"""
bridges/wechat.py — WeChat (iLink Bot API) bridge for Promethean.
        ...
        try:
            ...
            for msg in result.get("msgs") or []:
                ...
                if text.strip().lower() in ("/stop", "/off"):
                    _wx_send(from_uid, "🔴 promethean bridge stopped.", config)
                    _wechat_stop.set()
                    break

                if text.strip().lower() == "/start":
                    _wx_send(from_uid, "🟢 promethean bridge is active. Send me anything.", config)
                    continue
