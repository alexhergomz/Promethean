"""
bridges/telegram.py — Telegram bot bridge for Promethean.
...
def _tg_poll_loop(token: str, chat_id: int, config: dict) -> str:
    ...
    if flush and flush.get("ok") and flush.get("result"):
        ...
        offset = 0
    _tg_send(token, chat_id, "🟢 promethean is online.\nSend me a message and I'll process it.")
            ...
            for update in result.get("result", []):
                ...
                if text.strip().startswith("/"):
                    ...
                    if tg_cmd in ("/stop", "/off"):
                        ...
                        _tg_send(token, chat_id, "🟢 promethean bridge is active. Send me anything.")
                        continue
