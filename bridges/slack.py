"""
bridges/slack.py — Slack Web API bridge for Promethean.
...
def _slack_poll_loop(token: str, channel: str, config: dict) -> str:
    ...
    _slack_send(token, channel, "🟢 promethean is online. Send me a message and I'll process it.")
        ...
        try:
            ...
            for msg in messages:
                ...
                if text.strip().lower() in ("/stop", "/off"):
                    _slack_send(token, channel, "🔴 promethean bridge stopped.")
                    _slack_stop.set()
                    break

                if text.strip().lower() == "/start":
                    _slack_send(token, channel, "🟢 promethean bridge is active. Send me anything.")
                    continue
