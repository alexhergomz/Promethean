...
def _find_gemini_image_gen() -> str | None:
    ...
    # The promethean package lives at: .../promethean_versions/promethean/video/
    # v-content-creator lives at:         .../promethean_versions/v-content-creator/
    this_dir     = os.path.dirname(os.path.abspath(__file__))
    pkg_root     = os.path.dirname(this_dir)          # .../promethean
    versions_dir = os.path.dirname(pkg_root)           # .../promethean_versions
...
def _wikimedia_search(keywords: str, story_context: str = "") -> str | None:
    ...
    try:
        req = urllib.request.Request(api, headers={'User-Agent': 'Promethean/1.0'})
...
def _unsplash_search(keywords: str, width: int, height: int, access_key: str) -> str | None:
    ...
    try:
        req = urllib.request.Request(url, headers={'User-Agent': 'Promethean/1.0'})
        with urllib.request.urlopen(req, timeout=10) as resp:
            data = json.loads(resp.read())
