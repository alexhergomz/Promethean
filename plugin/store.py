...
USER_PLUGIN_DIR  = Path.home() / ".promethean" / "plugins"
USER_PLUGIN_CFG  = Path.home() / ".promethean" / "plugins.json"
...
# in the user config.
PLUGIN_PATH_ENV     = "PROMETHEAN_PLUGIN_PATH"
_EXTERNAL_ENABLED_KEY = "external_enabled"

def _project_plugin_dir() -> Path:
    return Path.cwd() / ".promethean" / "plugins"

def _project_plugin_cfg() -> Path:
    return Path.cwd() / ".promethean" / "plugins.json"
...
def _external_plugin_dirs() -> list[Path]:
    """Return existing directories listed in $PROMETHEAN_PLUGIN_PATH."""
...
def _scan_external_plugins() -> list[PluginEntry]:
    """Discover plugins sitting in-place under $PROMETHEAN_PLUGIN_PATH.
...
def list_plugins(scope: PluginScope | None = None) -> list[PluginEntry]:
    ...
    External plugins (from $PROMETHEAN_PLUGIN_PATH) are included unless a
