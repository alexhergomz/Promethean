# Architecture Guide — Harness Layer

This document is for contributors who want to understand, modify, or
extend the **harness layer** of Promethean — the *why* and *how* behind
the Python-side code, not the PR checklist. For the quick-start flow,
pointers on where to add things, and the PR checklist, see
[CONTRIBUTING.md](../CONTRIBUTING.md). For the user-facing surface
(CLI flags, slash commands, provider setup), see
[README.md](../README.md).

> The harness layer is forked from
> [cheetahclaws](https://github.com/SafeRL-Lab/cheetahclaws). Internal
> module / file names still use the `promethean` prefix; a rename is
> on the medium-term TODO list. The inference layer (TurboQuant +
> TriAttention `llama.cpp` patches) and the model layer (HQQ Qwen3.5
> -9B) live in separate sibling repos — this guide covers only the
> Python-side agent harness.

---

## Overview

The harness is a Python-native terminal coding agent that speaks to
any OpenAI-compatible endpoint (Anthropic, OpenAI, Gemini, Kimi, Qwen,
Zhipu, DeepSeek, MiniMax, Ollama, LM Studio, the local Promethean
`llama-server` build). It started as a ~900-line single-file script
in upstream cheetahclaws and has grown into a roughly 45 KLoC
multi-package codebase; the repository is in a **mostly-package
layout with intentional backward-compat shims** at the top level.
...
```
                        User Input
                            ...
                            ▼
   ┌───────────────────────────────────────────────────────────┐
   │  promethean.py  —  REPL, slash dispatch, permission UI   │
   └────┬──────────────────────────────┬───────────────────────┘
        │                              │
        │    ┌─────────────────────────┴──────────────┐
        │    │                                        │
...
imports from `promethean.py` or `agent.py` at module load time.
...
| Module | Role |
|---|---|
| [`promethean.py`](../promethean.py) | REPL shell, `COMMANDS` dispatch, permission prompt UI, streaming render, entry point (`main()`) |
...
| [`cc_config.py`](../cc_config.py) | Defaults + `~/.promethean/config.json` load/save.  Strips `_`-prefixed keys on save. |
...
- `~/.promethean/sessions/daily/YYYY-MM-DD/session_<ts>.json`
  (capped by `session_daily_limit`).
- `~/.promethean/sessions/history.json` (capped by
  `session_history_limit`).
- `~/.promethean/sessions/mr_sessions/session_latest.json` for
  `/resume`.
...
`promethean.py::main()` runs the CLI, parses args, calls
...
`promethean.py` by importing every `cmd_*` from `commands/*.py`.
...
Agent *types* are loaded from `~/.promethean/agents/<name>.md`
...
- User scope — `~/.promethean/memory/<slug>.md` (shared).
- Project scope — `.promethean/memory/<slug>.md` (per cwd).
...
`~/.promethean/mcp.json` (user scope) lists servers; `/mcp reload`
...
Persisted to `.promethean/tasks.json` per cwd.

Distinct from `TodoWrite` in other coding agents — Promethean
...
project `.promethean/skills/` → user `~/.promethean/skills/` →
...
Scopes: user (`~/.promethean/plugins/`) and project
(`.promethean/plugins/`).  `/plugin recommend [context]` scores the
...
`~/.promethean/agent_templates/`).  Built-in templates:
...
- Persists iteration records to
  `~/.promethean/agents/<name>/log.jsonl`.
...
command inside the REPL, or by `promethean --serve` (TBD).
...
`~/.promethean/config.json`.  It holds user settings (model,
...
```
 1. promethean.py            reads line via ui.input
...
16. promethean.py renders    prompt [y/N/a]; user types y → req.granted=True
...
- **`.nano_claude/plans/` vs `~/.promethean/`**: runtime state is
  under `~/.promethean/` (underscore), but plan mode writes to
