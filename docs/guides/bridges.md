...
`/telegram` turns promethean into a Telegram bot — receive messages from your phone, run the model with full tool access, and reply automatically.
...
3. Configure promethean (example with the values above):
...
Token and chat_id are saved to `~/.promethean/config.json`. On next launch the bridge **auto-starts** if configured — the startup banner shows `flags: [telegram]`.

### How it works

```
Phone (Telegram)                  promethean terminal
──────────────────                ──────────────────────────
...
- **Slash command passthrough**: send `/cost`, `/model gpt-4o`, `/clear`, `/monitor`, `/agent`, etc. from Telegram and they execute in promethean.
...
Send any photo (with or without a caption). Promethean downloads the highest-resolution version, encodes it as Base64, and passes it to the active vision model alongside the caption text. If no caption is provided, the default prompt is `"What do you see in this image? Describe it in detail."`.
...
Send a voice note (OGG) or audio file (MP3). Promethean transcribes it automatically and submits the transcript as your next query. The transcription is echoed back to the chat before the model responds.
...
If both `telegram_token` and `telegram_chat_id` are set in `~/.promethean/config.json`, the bridge starts automatically on every promethean launch:
...
<div align=center>
<center style="color:#000000;text-decoration:underline">WeChat Bridge: Control promethean from WeChat (微信)</center>
</div>

`/wechat` connects promethean to WeChat via **Tencent's iLink Bot API** — the same underlying protocol used by the official [WeixinClawBot](https://www.npmjs.com/package/@tencent-weixin/openclaw-weixin) plugin. Authenticate by scanning a QR code with your WeChat app; no manual token setup required.
...
Run `/wechat login` in promethean. A QR code URL appears in the terminal — open it in a browser or scan it directly if you installed the `qrcode` package:
...
Scan the QR code URL with WeChat. Once confirmed, the bridge starts immediately. Credentials (`token` + `base_url`) are saved to `~/.promethean/config.json` and reused on every subsequent launch — you only need to scan once.
...
```
Phone (WeChat)          promethean terminal
──────────────          ──────────────────────────────────
...
- **Slash command passthrough** — send `/cost`, `/model gpt-4o`, `/clear`, `/monitor`, `/agent`, etc. from WeChat and they execute in promethean. The result is sent back to the same WeChat conversation.
...
If `wechat_token` is set in `~/.promethean/config.json`, the bridge starts automatically on every promethean launch:
...
<div align=center>
<center style="color:#000000;text-decoration:underline">Slack Bridge: Control promethean from Slack</center>
</div>

`/slack` connects promethean to a Slack channel via the **Slack Web API** — no external packages required, just a Bot User OAuth Token and a channel ID. Messages are polled every 2 seconds using `conversations.history`; replies update an in-place "⏳ Thinking…" placeholder so the conversation feels responsive.
...
[myproject] ❯ /slack xoxb-12345-... C0123456789
  ...
  ℹ Slack authenticated as @promethean_bot
...
Credentials are saved to `~/.promethean/config.json` and the bridge auto-starts on every subsequent launch — you only need to configure once.

### How it works

```
Slack channel                    promethean terminal
─────────────                    ──────────────────────────────────
...
Every 2 seconds, promethean polls `GET conversations.history?oldest=<last_ts>`. When a message arrives, a `⏳ Thinking…` placeholder is posted immediately via `chat.postMessage`, then updated in-place with the real reply via `chat.update` once the model finishes.
...
- **Slash command passthrough** — send `/cost`, `/model gpt-4o`, `/clear`, `/monitor`, `/agent`, etc. from Slack and they execute in promethean; results are sent back to the same channel.
...
If `slack_token` and `slack_channel` are set in `~/.promethean/config.json`, the bridge starts automatically on every promethean launch:
...
```
Phone: "Run all tests"
promethean: ⏳ Queued as job #a3f2 (position 1)
              "Run all tests"
              Use !jobs to check status.

Phone: !jobs
promethean: 📊 Job Dashboard
              ────────────────────────────────────
...
Jobs are persisted to `~/.promethean/jobs.json` (last 100 kept).
