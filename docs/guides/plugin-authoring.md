...
Build and distribute plugins for Promethean. Plugins can add tools (callable by the AI), slash commands (typed by the user), skills (prompt templates), and MCP servers.

## Quick Start

```bash
# Create a plugin from the example template
cp -r examples/example-plugin ~/.promethean/plugins/my-plugin
# Edit the files, then restart promethean
promethean
...
```bash
/plugin install my-plugin@https://github.com/you/promethean-my-plugin
```
...
```json
{
  ...
  "homepage": "https://github.com/you/promethean-my-plugin"
}
```
...
|-------|-----------|--------|----------|
| **User** (default) | `~/.promethean/plugins/<name>/` | `~/.promethean/plugins.json` | Personal tools available everywhere |
| **Project** | `.promethean/plugins/<name>/` | `.promethean/plugins.json` | Project-specific tools, committed to git |
| **External** | Any dir listed in `$PROMETHEAN_PLUGIN_PATH` | enable state in `~/.promethean/plugins.json` | Shared team/company plugins, no install step |
...
## External Plugins (`PROMETHEAN_PLUGIN_PATH`)

External plugins are discovered **in-place** from directories you control — Promethean never copies them to `~/.promethean/plugins/`. This is the right fit for shared team or company plugin directories: the ops team maintains one source of truth, users just point an env var at it.

### Setup

```bash
# Single directory
export PROMETHEAN_PLUGIN_PATH=/opt/company/promethean-plugins

# Multiple directories (colon-separated on Linux/macOS, semicolon on Windows)
export PROMETHEAN_PLUGIN_PATH=/opt/company/plugins:$HOME/my-shared-plugins
```

Each **immediate subdirectory** with a `plugin.json` or `PLUGIN.md` is picked up:

```
/opt/company/promethean-plugins/
├── audit-tools/
│   ├── plugin.json
│   └── tools.py
├── company-skills/
...
Enable once — the decision persists to `~/.promethean/plugins.json` and survives restarts:
...
If the same plugin name exists in both installed (`USER`/`PROJECT`) and external scopes, the **installed** entry wins. Within external scopes, the **earliest** directory in `PROMETHEAN_PLUGIN_PATH` wins — same semantics as `$PATH`.

### Maintenance

- `/plugin uninstall <name>` on an external plugin only drops Promethean's enable-state record. **It never deletes the source directory** — that's the plugin author's to manage.
...
cp -r my-plugin ~/.promethean/plugins/my-plugin
...
3. Consider naming your repo `promethean-<name>` for discoverability
...
- [ ] Tested with `promethean` on Python 3.10+
...
| Importing `promethean` in plugin code | Use `config` parameter or `import runtime` for runtime state |
...
| Hardcoding file paths | Use `Path.home() / ".promethean"` or relative paths |
