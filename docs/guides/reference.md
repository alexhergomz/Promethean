...
```
promethean [OPTIONS] [PROMPT]
# or: python promethean.py [OPTIONS] [PROMPT]
...
promethean --model gpt-4o
promethean -m ollama/deepseek-r1:32b

# Non-interactive / scripting
promethean --print "Write a Python fibonacci function"
promethean -p "Explain the Rust borrow checker in 3 sentences" -m gemini/gemini-2.0-flash

# CI / automation (no permission prompts)
promethean --accept-all --print "Initialize a Python project with pyproject.toml"

# Debug mode (see tokens + thinking)
promethean --thinking --verbose
```
...
Tasks are saved to `.promethean/tasks.json` in the current working directory after every mutation and reloaded on first access.
...
**Install:** `pip install promethean[browser]` then `playwright install chromium`
...
**Install:** `pip install promethean[files]`
...
**Install:** `pip install promethean[ocr]` + system Tesseract
...
**Install:** `pip install promethean[files]` (for Excel; CSV works without any install)
