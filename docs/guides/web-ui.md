...
Promethean ships with a production-ready browser UI built on a pure Python stdlib HTTP server plus nine small vanilla-JS modules — no Node.js, no bundler, no build step. This guide covers installation, accounts, the Chat UI, the PTY terminal, the full HTTP API, observability, and how the pieces fit together.
...
pip install 'promethean[web]'

# Launch (auto-picks a free port if 8080 is taken):
promethean --web

# Explicit port / host / no-auth:
promethean --web --port 9000
promethean --web --host 0.0.0.0             # open to the local network
promethean --web --no-auth                  # localhost dev only — skips login
```

Startup banner:

```
  Promethean Web Terminal
  ────────────────────────────────────────
...
- JWT: **PyJWT**, HS256, **7-day TTL**. Signing secret is generated once and persisted to `~/.promethean/web_secret` (0600), so logins survive server restarts. Override with `PROMETHEAN_WEB_SECRET` env var.
...
- **DB file:** `~/.promethean/web.db` (0600). Override with `PROMETHEAN_WEB_DB`.
...
A full xterm.js (v5.5) terminal emulator in the browser — identical to running `promethean` in a native shell. 100% feature parity.
...
| `/metrics` | GET | Prometheus v0.0.4 text. Exports `promethean_uptime_seconds`, `promethean_requests_total`, `promethean_requests_4xx`, `promethean_requests_5xx`, `promethean_auth_logins_total`, `promethean_auth_logins_failed`, `promethean_auth_registrations_total`, `promethean_users_total`, `promethean_ws_connections_total` |
...
Other structured events: `server_start`, `server_stop`, `register`, `login`, `login_failed`, `db_init_failed`, `message persist failed` (from `web.api`). Level controlled by `PROMETHEAN_LOG_LEVEL` (default `INFO`; set `DEBUG` for verbose).
...
|----------|---------|---------|
| `PROMETHEAN_WEB_DB` | `~/.promethean/web.db` | SQLite file path |
| `PROMETHEAN_WEB_SECRET` | persisted to `~/.promethean/web_secret` | JWT HS256 signing key |
| `PROMETHEAN_LOG_LEVEL` | `INFO` | `DEBUG` / `INFO` / `WARNING` / `ERROR` |
| `PROMETHEAN_WEB_SERVER` | set by `start_web_server` to `1` | Guards against recursive `--web` launches via shell aliases |
...
**8080 is taken**
`promethean --web` (with no `--port`) auto-falls back to a free port — check the banner for the real URL. If you must use 8080, stop the conflicting process first.
...
Blow away the SQLite DB and re-register: `rm ~/.promethean/web.db` then restart. You'll lose all chat history — for real recovery, open the DB with any SQLite client and rewrite the `password_hash` (`bcrypt.hashpw(b"newpass", bcrypt.gensalt()).decode()`).
...
The log line is JSON with the full exception. Usually a file-permission issue on `~/.promethean/web.db` or a broken install of SQLAlchemy. Verify `pip install 'promethean[web]'` completed without errors.
