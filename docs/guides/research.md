...
and **auto-saved reports** to `~/.promethean/research_reports/` with
...
- `PROMETHEAN_GITHUB_TOKEN` (or `GITHUB_TOKEN`) — raises GitHub search limits from 10/min → 60/min
...
| `--no-cache` | Skip the 24h SQLite cache at `~/.promethean/research_cache.db`. |
| `--no-save` | Skip auto-save to `~/.promethean/research_reports/`. |
...
4. Writes a saved report (under `~/.promethean/research_reports/`)
...
`~/.promethean/research_reports/<YYYY-MM-DD_HHMMSS>-<slug>.md` plus
...
Results are cached in `~/.promethean/research_cache.db` (SQLite) with
