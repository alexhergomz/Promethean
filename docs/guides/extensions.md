...
|---|---|---|
| **User** (default) | `~/.promethean/memory/` | Shared across all projects |
| **Project** | `.promethean/memory/` in cwd | Local to the current repo |
...
```bash
mkdir -p ~/.promethean/skills
```

Create `~/.promethean/skills/deploy.md`:
...
```
./.promethean/skills/     # project-level (overrides user-level)
~/.promethean/skills/     # user-level
```
...
**Custom agent types** — create `~/.promethean/agents/myagent.md`:
```markdown
---
name: myagent
...
Place a `.mcp.json` file in your project directory **or** edit `~/.promethean/mcp.json` for user-wide servers.
...
Config priority: `.mcp.json` (project) overrides `~/.promethean/mcp.json` (user) by server name.
...
The `plugin/` package lets you extend promethean with additional tools, skills, and MCP servers from git repositories or local directories.
...
|-------|----------|--------|
| user (default) | `~/.promethean/plugins/` | `~/.promethean/plugins.json` |
| project | `.promethean/plugins/` | `.promethean/plugins.json` |
...
`/monitor` turns promethean into a 24/7 AI research assistant that watches stocks, crypto, arxiv, world news, or any custom topic on a schedule and pushes AI-written reports to Telegram, Slack, or your terminal.
...
✓ Agent 'research' is running.
  Log  : ~/.promethean/agents/research/log.jsonl
...
Drop any `.md` file into `~/.promethean/agent_templates/` following the program.md pattern:
...
Iteration log is also persisted to `~/.promethean/agents/<name>/log.jsonl` for offline review.
