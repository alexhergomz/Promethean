...
Promethean v3.05 adds a fully offline voice-to-prompt pipeline. Speak your request — it is transcribed and submitted as if you had typed it.
...
Before each recording, Promethean extracts coding vocabulary from:
- **Git branch** (e.g. `feat/voice-input` → "feat", "voice", "input")
- **Project root name** (e.g. "promethean")
...
[0] Content mode
  ...
  Promethean is a lightweight Python AI coding assistant
...
- Font is downloaded once to `~/.promethean/fonts/` on first run (~8 MB)
