...
**Goal:** Evolve promethean into a capable AI coding CLI with context management, pluggable tools, sub-agents, memory, skills, and diff view.
...
- `promethean.py` — Add /memory, /skill, /agents slash commands, diff rendering
...
Run: `cd D:/git/open-cc/promethean && python -m pytest tests/test_tool_registry.py -v`
...
Run: `cd D:/git/open-cc/promethean && python -c "from tools import execute_tool; from tool_registry import get_all_tools; print(f'{len(get_all_tools())} tools registered'); print(execute_tool('Read', {'file_path': 'config.py'}, 'auto'))"`
...
## Task 3: Diff View (`tools.py` + `promethean.py`)

**Files:**
- Modify: `tools.py` (the `_edit` and `_write` functions)
- Modify: `promethean.py` (the `print_tool_end` function)
...
Run: `cd D:/git/open-cc/promethean && python -m pytest tests/test_diff_view.py -v`
...
- [ ] **Step 6: Add diff rendering to promethean.py**
...
Also ensure the color dict `C` (line 60-70 of promethean.py) includes the needed keys. Check and add if missing:
...
Run: `cd D:/git/open-cc/promethean && python -c "
...
```bash
cd D:/git/open-cc/promethean
git add tools.py promethean.py tests/test_diff_view.py
...
Run: `cd D:/git/open-cc/promethean && python -m pytest tests/test_compaction.py -v`
...
# compaction.py
"""
Context window management for promethean.
...
Run: `cd D:/git/open-cc/promethean && python -m pytest tests/test_memory.py -v`
...
# memory.py
"""
File-based memory system for promethean.
...
MEMORY_DIR = Path.home() / ".promethean" / "memory"
...
Run: `cd D:/git/open-cc/promethean && python -m pytest tests/test_subagent.py -v`
...
# subagent.py
"""
Sub-agent lifecycle management for promethean.
...
- Modify: `promethean.py` (skill dispatch in REPL)
...
Run: `cd D:/git/open-cc/promethean && python -m pytest tests/test_skills.py -v`
...
# skills.py
"""
Skill loading and execution for promethean.
...
SKILL_PATHS = [
    Path.cwd() / ".promethean" / "skills",
    Path.home() / ".promethean" / "skills",
]
...
- [ ] **Step 5: Integrate skills into promethean.py REPL**

In `promethean.py`, add import near the top:
...
def cmd_skills(_args, _state, _config):
    skills = load_skills()
    if not skills:
        info("No skills found. Place .md files in ~/.promethean/skills/")
        return True
    for s in skills:
...
```bash
cd D:/git/open-cc/promethean
git add skills.py tests/test_skills.py promethean.py
...
- Modify: `promethean.py` (add /memory, /agents commands)
...
Run: `cd D:/git/open-cc/promethean && python -m pytest tests/ -v`
...
```bash
cd D:/git/open-cc/promethean
git add config.py promethean.py
...
Run: `cd D:/git/open-cc/promethean && python -m pytest tests/ -v --tb=short`
