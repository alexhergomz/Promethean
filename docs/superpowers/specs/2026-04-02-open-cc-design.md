# Open-CC: Promethean Enhancement Design
...
Evolve promethean from a minimal ~2.2K-line reference implementation into a capable AI coding CLI, approaching Claude Code's core functionality while staying lean. Five enhancement areas:
...
```
promethean/
├── promethean.py      # REPL -- add /memory, /skill slash commands
...
In `promethean.py`, unmatched `/` commands fall through to skill lookup:
...
### 8.2 REPL Rendering (in promethean.py)
...
| 2 | Diff view in `tools.py` + `promethean.py` | Phase 1 | ~100 |
...
| 6 | `skills.py` + promethean.py integration | Phase 1, 4 | ~200 |
