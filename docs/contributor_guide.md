# Contributor Guide: Where to Change What in promethean
...
1. `promethean.py` handles CLI + REPL + slash commands.
...
- REPL command wiring lives in `promethean.py` (`cmd_checkpoint`, `cmd_rewind`).
...
- REPL command wiring lives in `promethean.py` (`cmd_voice`).
...
1. Add `cmd_<name>` function in `promethean.py`.
...
3. Add slash command wiring in `promethean.py` only if user-facing command is needed.
...
- **Path naming note:** most runtime dirs use `.promethean` (underscore).
