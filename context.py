...
def _agent_tools_available() -> bool:
    """Detect if the agent_tools package is importable (Aider-style repo map).
    The codeact fragment is appended to the prompt only when this is true."""
    try:
...
def build_system_prompt(config: dict | None = None) -> str:
    ...
    # Skills catalog: surface reusable task playbooks so the model can invoke
    # the right one (via the Skill tool) without first calling SkillList.
    try:
        from skill.loader import skills_catalog
        catalog = skills_catalog()
        if catalog:
            parts.append(
                "# Skills\n"
                "Reusable task playbooks available via the **Skill** tool "
                "(call it with the skill's name). Invoke one when the user's "
                "request matches its when-to-use:\n" + catalog
            )
    except Exception:
        pass
    ...
    if _agent_tools_available():
        parts.append(load_fragment("codeact"))
