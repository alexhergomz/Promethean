...
persisted to ~/.promethean/web_secret so restarts don't invalidate sessions.
...
try:
    ...
    raise ImportError(
        ...
        "Install it with: pip install 'promethean[web]'"
    ) from exc
...
_SECRET_PATH = Path.home() / ".promethean" / "web_secret"


def _load_or_create_secret() -> str:
    """Return a stable 32-byte URL-safe secret, creating it on first use."""
    env = os.environ.get("PROMETHEAN_WEB_SECRET")
    if env:
        return env
    try:
