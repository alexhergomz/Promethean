"""Lightweight web terminal server for Promethean.

Spawns a Promethean REPL in a PTY and bridges it to the browser.
...
<title>Promethean Web Terminal</title>
...
<div id="topbar">
  <span class="logo">Promethean Web Terminal</span>
  <span class="status">
    ...
    try:
        ...
        if path == "/metrics" and method == "GET":
            ...
            try:
                ...
                pass
            lines: list[str] = []
            lines.append("# HELP promethean_uptime_seconds Server uptime")
            lines.append("# TYPE promethean_uptime_seconds gauge")
            lines.append(f"promethean_uptime_seconds "
                         f"{round(uptime_seconds(), 3)}")
            for k, v in {**counters, **extras}.items():
                lines.append(f"# HELP promethean_{k} {k}")
                lines.append(f"# TYPE promethean_{k} counter")
                lines.append(f"promethean_{k} {v}")
            body = ("\n".join(lines) + "\n").encode()
...
def _bind_port(host: str, preferred: Optional[int]) -> tuple[socket.socket, int]:
    ...
    `promethean --web` Just Works even when 8080 is in use. When the user
...
def start_web_server(
    ...
    if os.environ.get("PROMETHEAN_WEB_SERVER") == "1":
        ...
        sys.exit(1)
    os.environ["PROMETHEAN_WEB_SERVER"] = "1"
    ...
    try:
        ...
        if _log:
            _log.warning("chat_ui_disabled_missing_deps",
                         extra={"err": str(exc),
                                "hint": "pip install 'promethean[web]'"})
        else:
            print(f"\033[33m[web] Chat UI disabled — missing deps: {exc}.\n"
                  f"      Install with: pip install 'promethean[web]'\033[0m",
                  file=sys.stderr)
        _chat_user_count = 0
    except Exception as exc:  # noqa: BLE001
        if _log:
    ...
    cc_bin = shutil.which("promethean")
    if cc_bin:
        _server_cmd = [cc_bin]
    else:
        cc_script = Path(__file__).resolve().parent.parent / "promethean.py"
    ...
    print(f"\n  \033[36mPromethean Web Terminal\033[0m", flush=True)
